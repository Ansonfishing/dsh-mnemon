	...
	factory: (require) => {
		...
		function MnemonWorkspace({ connection, settingsScope, sessionId, workspaceId, workspaceSelection }) {
			...
			const [statusState, setStatusState] = (0, react.useState)(() => ({
				contextKey: clientContextKey,
				value: null,
				loading: true,
				error: null
			}));
			const currentStatusState = statusState.contextKey === clientContextKey ? statusState : {
				contextKey: clientContextKey,
				value: null,
				loading: true,
				error: null
			...
			const loadStatus = (0, react.useCallback)(async () => {
				...
				setStatusState((current) => ({
					contextKey: clientContextKey,
					value: current.contextKey === clientContextKey ? current.value : null,
					loading: true,
					error: null
				}));
				try {
			...
			(0, react.useEffect)(() => {
				loadStatus();
			}, [loadStatus, settingsSnapshot.revision]);
