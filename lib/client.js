window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function naturalGraphPositions(nodes, edges) {
			...
			for (const [category, items] of grouped) {
				...
				items.forEach((node, index) => {
					...
					positions.set(node.id, clampGraphPosition({
						x: anchor.x + Math.cos(angle) * radius,
						y: anchor.y + Math.sin(angle) * radius
					}));
				});
			}
			const velocities = new Map(nodes.map((node) => [node.id, {
				x: 0,
				y: 0
			}]));
		...
		function MemoryGraph(props) {
			...
			const visibleIds = (0, react.useMemo)(() => new Set(visibleNodes.map((node) => node.id)), [visibleNodes]);
				...
				children: [
					...
					visibleNodes.map((node, index) => {
						...
						return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("g", {
							...
							"data-dragging": dragRef.current?.nodeId === node.id || void 0,
