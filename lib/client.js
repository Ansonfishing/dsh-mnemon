window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"status.workerCounts": "召回 {recalls} · 问答 {answers} · 审查 {reviews} · 写入 {writes}",
			"status.lifecycle": "子 Agent 生命周期",
			"status.lifecycleText": "pre-step 只提供短提示；主 Agent 自主调用工具，持续空闲后再由 fork 子 Agent 审查完整 checkpoint。",
			"status.phaseIdle": "待命",
			...
			"status.writeText": "连续空闲 {seconds} 秒后 fork 完整 checkpoint，按需 remember / forget",
		...
		const en = {
			...
			"status.workerCounts": "{recalls} recalls · {answers} answers · {reviews} reviews · {writes} writes",
			"status.lifecycle": "Subagent Lifecycle",
			"status.lifecycleText": "Pre-step adds only a short cue; the main Agent calls tools by need, then a fork child reviews the full checkpoint after sustained idle.",
			...
			"status.writeText": "Fork the full checkpoint after {seconds}s idle, then remember or forget only when justified",
