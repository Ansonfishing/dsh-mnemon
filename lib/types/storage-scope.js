...
function runtimeArea(root) {
    ...
    try {
        ...
        if (file === undefined || !Array.isArray(file.entries))
            throw new Error('memories.json is not a valid runtime-memory source');
        const entries = file.entries.map(record).filter((entry) => entry !== undefined);
...
function memoryBodiesArea(root) {
    ...
    if (!existsSync(path))
        return missing('memory-bodies', path);
    try {
        ...
        const bodies = Array.isArray(registry?.bodies) ? registry.bodies.map(record).filter((body) => body !== undefined) : [];
...
function documentsArea(root) {
    ...
    if (!existsSync(indexPath))
        return missing('documents', path);
    try {
        const index = record(readJson(indexPath));
        if (index === undefined || !Array.isArray(index.documents))
            throw new Error('index.json is not a valid Documents index');
        const documents = index.documents.map(record).filter((document) => document !== undefined);
...
function inspect(kind, rawRoot, activeRoot) {
    if (rawRoot === undefined)
        return { kind, configured: false, active: false, available: false, totalBytes: 0, areas: [], issue: 'scope is not configured' };
    const root = canonical(rawRoot);
    const areas = [runtimeArea(root), memoryBodiesArea(root), documentsArea(root), stateArea(root)];
    const exists = existsSync(root);
    const available = exists && (() => { try {
...
export function validateCustomStorageRoot(value) {
    const path = value.trim();
    if (path === '')
        throw new Error('custom storage directory is required');
    const expanded = expandHome(path);
    if (!isAbsolute(expanded))
        throw new Error('custom storage directory must be an absolute path or start with ~/');
