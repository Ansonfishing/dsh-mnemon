...
export function MnemonSettingsCard({ scope, interactionScope: suppliedInteractionScope, connection, pickDirectory, t = translateZh }) {
    ...
    const resetFields = (fields) => { for (const field of fields)
        if (overridden(field))
            resetField(field); };
    const browse = async () => {
        if (pickDirectory === undefined || browsing)
            return;
        setBrowsing(true);
        setFailed(null);
        try {
    ...
    return (_jsx("section", { className: css.page, "aria-label": t('config.aria'), "aria-busy": saving || loading || browsing, children: loading ? _jsx("p", { className: css.loading, role: "status", children: t('common.loading') }) : _jsxs(_Fragment, { children: [_jsxs("header", { className: css.pageHeader, children: [_jsx("h1", { children: t('config.title') }), _jsx("p", { children: t('config.description') })] }), _jsxs("section", { className: css.section, "aria-labelledby": "mnemon-storage-heading", children: [_jsxs("div", { className: css.sectionHeading, children: [_jsxs("div", { children: [_jsx("h2", { id: "mnemon-storage-heading", children: t('config.storageTitle') }), _jsxs("p", { children: [t('config.storageDescription'), " ", t('config.restart')] })] }), storageOverridden && _jsx("button", { className: css.textButton, type: "button", disabled: coreDisabled, onClick: () => resetFields(CORE_FIELDS), children: t('config.reset') })] }), _jsxs("div", { className: css.choiceGrid, role: "radiogroup", "aria-label": t('config.scopeAria'), children: [_jsx(ChoiceCard, { id: "mnemon-storage-global", name: "mnemon-storage", label: t('config.global'), detail: "~/.mnemon", checked: draft.storageScope === 'global', disabled: coreDisabled, onChange: () => edit('storageScope', 'global') }), _jsx(ChoiceCard, { id: "mnemon-storage-workspace", name: "mnemon-storage", label: t('config.workspace'), detail: "<workspace>/.mnemon", checked: draft.storageScope === 'workspace', disabled: coreDisabled, onChange: () => edit('storageScope', 'workspace') }), _jsx(ChoiceCard, { id: "mnemon-storage-custom", name: "mnemon-storage", label: t('config.custom'), detail: draft.dataDir === '' ? t('config.customHintShort') : t('config.customSelected'), checked: draft.storageScope === 'custom', disabled: coreDisabled, onChange: () => { edit('storageScope', 'custom'); if (draft.dataDir === '')
                                        void browse(); } })] }), draft.storageScope === 'custom' && _jsxs("div", { className: css.settingRow, children: [_jsxs("div", { className: css.settingCopy, children: [_jsx("strong", { children: t('config.customDirectory') }), _jsx("small", { children: t('config.customBrowseHint') })] }), _jsxs("div", { className: css.directoryControl, children: [_jsx("code", { title: draft.dataDir, children: draft.dataDir || t('config.customNotSelected') }), _jsx("button", { type: "button", className: css.pillButton, disabled: coreDisabled || pickDirectory === undefined, onClick: () => void browse(), children: browsing ? t('config.customBrowsing') : draft.dataDir === '' ? t('config.customBrowse') : t('config.customBrowseAgain') })] })] })] }), _jsxs("section", { className: css.section, "aria-labelledby": "mnemon-interaction-heading", children: [_jsxs("div", { className: css.sectionHeading, children: [_jsxs("div", { children: [_jsx("h2", { id: "mnemon-interaction-heading", children: t('config.interactionTitle') }), _jsx("p", { children: t('config.interactionHint') })] }), interactionOverridden && _jsx("button", { className: css.textButton, type: "button", disabled: interactionDisabled, onClick: () => resetFields(INTERACTION_FIELDS), children: t('config.reset') })] }), _jsxs("div", { className: css.rowGroup, children: [_jsx(ToggleRow, { id: "mnemon-interaction-toolviews", label: t('config.interactionToolviews'), hint: t('config.interactionToolviewsHint'), checked: draft.toolviews, disabled: interactionDisabled, onChange: value => edit('toolviews', value) }), _jsx(ToggleRow, { id: "mnemon-interaction-turn-bar", label: t('config.interactionTurnBar'), hint: t('config.interactionTurnBarHint'), checked: draft.turnBar, disabled: interactionDisabled, onChange: value => edit('turnBar', value) }), _jsx(ToggleRow, { id: "mnemon-interaction-save-action", label: t('config.interactionSaveAction'), hint: t('config.interactionSaveActionHint'), checked: draft.saveAction, disabled: interactionDisabled, onChange: value => edit('saveAction', value) })] })] }), _jsx(MnemonPackSection, { ...(connection === undefined ? {} : { connection }), configuredScope: draft.storageScope, configuredDirectory: draft.dataDir, storageDirty: CORE_FIELDS.some(field => dirty.has(field)), t: t }), _jsxs("div", { className: css.feedback, "aria-live": "polite", children: [error !== null && _jsx("p", { className: css.error, role: "alert", children: error }), failed !== null && _jsx("p", { className: css.error, role: "alert", children: t('config.saveFailed', { error: failed }) }), pickDirectory === undefined && draft.storageScope === 'custom' && _jsx("p", { className: css.readOnly, children: t('config.customBrowseUnavailable') }), !writable && _jsx("p", { className: css.readOnly, children: t('config.readOnly') })] }), _jsxs("footer", { className: `${css.actions} ${dirty.size > 0 ? css.actionsVisible : ''}`, "aria-live": "polite", children: [_jsx("span", { children: t('config.unsaved') }), _jsxs("div", { children: [_jsx("button", { type: "button", className: css.discard, disabled: saving, onClick: discard, children: t('config.discard') }), _jsx("button", { type: "button", className: css.save, disabled: saving || error !== null || !writable, onClick: () => void save(), children: saving ? t('config.saving') : t('config.save') })] })] }), _jsxs("p", { className: css.settingsNote, children: [t('config.noticeBefore'), " ", _jsx("code", { children: ".dsh/settings.yaml" }), t('config.noticeAfter')] })] }) }));
}
