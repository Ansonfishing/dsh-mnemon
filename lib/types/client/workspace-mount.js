...
import { mountMnemonSidebarEntry } from "./sidebar-entry.js";
import { MnemonWorkspaceController } from "./workspace-controller.js";
...
function MnemonPanel({ ctx, settings, t, onClose }) {
    const subscribeLocale = useCallback((listener) => ctx.locale.subscribe(listener), [ctx.locale]);
    ...
    useEffect(() => {
        if (resolvedSelectedId !== selectedWorkspaceId)
            setSelectedWorkspaceId(resolvedSelectedId);
    ...
    const selection = useMemo(() => ({
        ...
        onAlign: () => {
            if (effectiveWorkspace !== undefined)
                setSelectedWorkspaceId(String(effectiveWorkspace.workspaceId));
        },
    }), [effectiveWorkspace, resolvedSelectedId, workspaces.items]);
    return _jsx(MnemonView, { connection: ctx.connection, settingsScope: settings, ...(sessions.current === undefined ? {} : { sessionId: sessions.current }), ...(resolvedSelectedId === undefined ? {} : { workspaceId: resolvedSelectedId }), workspaceSelection: selection, surface: "sidebar", t: t, locale: locale.active, onClose: onClose });
}
...
function mountPanel(controller, ctx, settings, t) {
    ...
    const ensure = () => {
        ...
        root.render(_jsx(MnemonPanel, { ctx: ctx, settings: settings, t: t, onClose: () => { controller.close(); } }));
    };
    ...
    const onOtherPanelActivate = (event) => {
        if (suppressCompatibilityClose || !controller.getSnapshot().open)
            return;
        const detail = event.detail;
        if (detail === 'taskboard' || detail === 'ssh')
            controller.close();
    };
    const onSidebarContextClick = (event) => {
        ...
        if (target instanceof Element && target.closest(SIDEBAR_CONTEXT_SELECTOR) !== null)
            controller.close();
    };
    const onAnchor = () => { controller.open(); };
...
export function mountMnemonWorkspace(ctx, settings, t) {
...
//# sourceMappingURL=workspace-mount.js.map