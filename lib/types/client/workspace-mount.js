...
function MnemonPanel({ ctx, settings, t }) {
    ...
    return _jsx(MnemonView, { connection: ctx.connection, settingsScope: settings, ...(sessions.current === undefined ? {} : { sessionId: sessions.current }), ...(resolvedSelectedId === undefined ? {} : { workspaceId: resolvedSelectedId }), workspaceSelection: selection, surface: "sidebar", t: t, locale: locale.active });
}
...
function mountPanel(controller, ctx, settings, t) {
    ...
    const ensure = () => {
        ...
        root.render(_jsx(MnemonPanel, { ctx: ctx, settings: settings, t: t }));
    };
