...
export function createReadHandler(service, lifecycle, runtimeMemory, storage) {
...
export function createPackHandler(manager, writeEnabled = true) {
    return async (endpoint, rawPayload) => {
        try {
            ...
            if (endpoint === 'export')
                return success(await manager.exportPack(String(payload.scope ?? '')));
            if (endpoint === 'inspect')
...
export function registerRpc(connection, service, lifecycle, runtimeMemory, storage, packs) {
    ...
    if (packs !== undefined)
        connection.rpc.handle(MNEMON_PACK_CHANNEL, createPackHandler(packs, service.config.writeEnabled), { authority: 'loopback' });
}
