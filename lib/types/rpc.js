import { VersionUpdateManager } from "./version-updates.js";
import { MNEMON_PACK_CHANNEL, MNEMON_READ_CHANNEL, MNEMON_WRITE_CHANNEL } from "./channels.js";
export { MNEMON_PACK_CHANNEL, MNEMON_READ_CHANNEL, MNEMON_WRITE_CHANNEL } from "./channels.js";
...
function requireAligned(route) {
    if (route?.aligned === false)
        throw new Error('the selected memory workspace differs from the current session; align the workbench before running an Agent-backed operation');
}
...
export function createReadHandler(input, lifecycle, runtimeMemory, storage, versions) {
    return async (endpoint, rawPayload) => {
        try {
            const payload = object(rawPayload);
            if (endpoint === 'versions') {
                if (versions === undefined)
                    throw new Error('version checks are unavailable');
            ...
            switch (endpoint) {
                case 'runtime-memory':
                    if (resolved.graph.runtimeMemory === undefined)
                ...
                case 'documents':
                    if (documentController !== undefined)
                        return success(documentController.snapshot());
                ...
                case 'document':
                    if (documentController !== undefined)
                        return success(documentController.get(String(payload.id ?? '')));
                    if (lifecycle === undefined)
                        throw new Error('Mnemon Documents require lifecycle integration');
                    return success(lifecycle.document(String(payload.sessionId ?? ''), String(payload.id ?? '')));
                case 'document-search':
                    if (documentController !== undefined)
                        return success(await documentController.search(String(payload.query ?? ''), { includeArchived: payload.includeArchived === true, ...(payload.limit === undefined ? {} : { limit: Number(payload.limit) }) }));
                    if (lifecycle === undefined)
                        throw new Error('Mnemon Documents require lifecycle integration');
                ...
                case 'turn-activities':
                    if (lifecycle === undefined)
                        throw new Error('Mnemon turn activity requires lifecycle integration');
                ...
                case 'assistant-message':
                    if (lifecycle === undefined)
                        throw new Error('Mnemon assistant message requires lifecycle integration');
...
export function createWriteHandler(input, lifecycle, runtimeMemory, versions) {
    return async (endpoint, rawPayload) => {
        try {
            const payload = object(rawPayload);
            if (endpoint === 'version-update') {
                if (versions === undefined)
                    throw new Error('version updates are unavailable');
                const component = String(payload.component ?? '');
                if (component !== 'mnemon' && component !== 'dsh-mnemon')
                    return badRequest(`unknown version component: ${component}`);
                return success(await versions.update(component));
            }
            ...
            switch (endpoint) {
                ...
                case 'supervise':
                    ...
                    return success(await lifecycle.supervise(String(payload.sessionId ?? ''), String(payload.content ?? ''), payload.idempotencyKey === undefined ? undefined : String(payload.idempotencyKey)));
                case 'document':
                    {
                        ...
                        if (documentController !== undefined && action !== 'archive' && (!alignedSession || lifecycle === undefined)) {
                            ...
                            if (action === 'create')
                                return success(await documentController.mutate({
                                    action: 'create',
...
export function createPackHandler(manager, writeEnabled = true) {
    return async (endpoint, rawPayload) => {
        try {
            const payload = object(rawPayload);
            if (endpoint === 'target')
                return success(manager.target());
            if (endpoint === 'export')
                return success(await manager.exportPack('full'));
            if (endpoint === 'inspect')
                return success(manager.inspectPack(String(payload.base64 ?? ''), payload.fileName === undefined ? undefined : String(payload.fileName)));
            if (endpoint === 'import') {
                const writable = typeof writeEnabled === 'function' ? writeEnabled() : writeEnabled;
                if (!writable)
                    throw new Error('Mnemon Pack import is disabled while memory writes are read-only');
...
export function registerRpc(connection, input, lifecycle, runtimeMemory, storage, packs, versions) {
    ...
    if (packManager !== undefined)
        connection.rpc.handle(MNEMON_PACK_CHANNEL, createPackHandler(packManager, () => config.writeEnabled), { authority: 'loopback' });
}
