...
import { findMnemonCommand } from "./runner.js";
...
async function settledWithin(promise, fallback, timeoutMs = CHECK_TIMEOUT_MS + 1_000) {
    let timer;
    try {
...
function linkedProfile(packageManifestPath, dshHome) {
    ...
    for (const entry of readdirSync(profilesDir, { withFileTypes: true })) {
        ...
        if (target === undefined || !existsSync(target))
            continue;
        try {
            if (realpathSync(target) === packageRoot)
                matches.push({ name: entry.name, dir: profileDir, locationDir: target });
        }
        catch {
    ...
    async update(component) {
        if (component === 'mnemon') {
            ...
            if (before.current === undefined)
                throw new Error('Mnemon CLI is unavailable');
            if (latest === undefined)
                throw new Error('Unable to verify the latest Mnemon release');
            if (compareVersions(before.current, latest) >= 0)
                return { component, previousVersion: before.current, currentVersion: before.current, updated: false, restartRequired: false };
            if (before.install.updateCommand === undefined || before.install.updateArgs === undefined)
                throw new Error('This Mnemon installation cannot be updated automatically');
        ...
        if (component !== 'dsh-mnemon')
            throw new Error(`Unknown version component: ${String(component)}`);
        const latest = await this.fetchNpmLatest(DSH_MNEMON_PACKAGE);
        if (latest === undefined)
            throw new Error('Unable to verify the latest dsh-mnemon release');
        if (compareVersions(this.currentDshMnemonVersion, latest) >= 0)
            return { component, previousVersion: this.currentDshMnemonVersion, currentVersion: this.currentDshMnemonVersion, updated: false, restartRequired: false };
        const install = inspectDshInstall(this.packageManifestPath, this.dshHome);
        const pnpm = this.executable('pnpm');
        if (install.mode !== 'npm' || install.profileDir === undefined || pnpm === undefined)
            throw new Error('This dsh-mnemon installation cannot be updated automatically');
...
//# sourceMappingURL=version-updates.js.map