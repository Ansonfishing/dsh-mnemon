...
export class DocumentController {
    ...
    constructor(workspaceRoot, limitBytes = DOCUMENTS_ACTIVE_LIMIT_BYTES, now = () => new Date(), storageRoot) {
    ...
    normalizeSourcePaths(paths) {
        return unique(paths, 50).map(value => {
            ...
            if (absolute === this.directory || absolute.startsWith(`${this.directory}${sep}`))
...
export class DocumentManager {
    ...
    storageRoot;
    controllers = new Map();
    constructor(limitBytes = DOCUMENTS_ACTIVE_LIMIT_BYTES, now = () => new Date(), storageRoot) {
        this.limitBytes = limitBytes;
        this.now = now;
