...
function normalizeLine(value, field, maximum, required) {
    ...
    if (normalized.length > maximum)
        throw new Error(`${field} is too long (max ${maximum} characters)`);
    return normalized;
}
function normalizeContent(value, required) {
    ...
    if (size > MAX_DOCUMENT_BYTES)
        throw new Error(`document content is too large (${size} bytes; max ${MAX_DOCUMENT_BYTES})`);
    return normalized;
}
...
function parseRecord(value) {
    if (!isRecord(value) || typeof value.id !== 'string' || typeof value.title !== 'string' || typeof value.description !== 'string')
        return undefined;
    if ((value.status !== 'active' && value.status !== 'archived') || typeof value.filename !== 'string' || typeof value.relativePath !== 'string')
        return undefined;
    if (typeof value.createdAt !== 'string' || typeof value.updatedAt !== 'string' || typeof value.lastAccessedAt !== 'string')
        return undefined;
    if (typeof value.revision !== 'number' || typeof value.contentHash !== 'string' || typeof value.sizeBytes !== 'number')
        return undefined;
    if (!Array.isArray(value.sourcePaths) || !Array.isArray(value.sessionIds) || !Array.isArray(value.memoryBodyIds))
        return undefined;
    return {
        id: value.id,
        title: value.title,
...
export class DocumentController {
    ...
    constructor(workspaceRoot, limitBytes = DOCUMENTS_ACTIVE_LIMIT_BYTES, now = () => new Date()) {
        this.limitBytes = limitBytes;
        this.now = now;
        this.workspaceRoot = resolve(workspaceRoot);
        if (!existsSync(this.workspaceRoot) || !statSync(this.workspaceRoot).isDirectory())
            throw new Error(`document workspace is unavailable: ${this.workspaceRoot}`);
        if (!Number.isSafeInteger(limitBytes) || limitBytes < 1)
            throw new Error('active document limit must be a positive integer');
        ...
        return this.withLock(() => {
            ...
            if (request.action === 'create') {
                ...
                if (current.status !== 'active')
                    throw new Error('archived documents are immutable; create a new active revision instead');
        ...
        const operation = this.queue.then(() => this.withLock(() => {
            ...
            const ranked = index.documents
                ...
                for (const token of tokens)
                    score += title.includes(token) ? 4 : description.includes(token) ? 2 : content.includes(token) ? 1 : 0;
    ...
    initialize() {
        ...
        if (!existsSync(this.indexPath))
            this.atomicWrite(this.indexPath, `${JSON.stringify({ version: DOCUMENTS_VERSION, documents: [] }, null, 2)}\n`);
        this.readIndex();
    }
    readIndex() {
        const raw = JSON.parse(readFileSync(this.indexPath, 'utf8'));
        if (!isRecord(raw) || raw.version !== DOCUMENTS_VERSION || !Array.isArray(raw.documents))
            throw new Error(`invalid document index: ${this.indexPath}`);
        const documents = raw.documents.map(parseRecord);
        if (documents.some(record => record === undefined))
            throw new Error(`invalid document record in ${this.indexPath}`);
        return { version: DOCUMENTS_VERSION, documents: documents };
    }
    ...
    normalizeSourcePaths(paths) {
        return unique(paths, 50).map(value => {
            ...
            if (workspaceRelative === join('.mnemon', 'documents') || workspaceRelative.startsWith(`${join('.mnemon', 'documents')}${sep}`))
                throw new Error('managed document paths cannot be used as source paths');
    ...
    pathFor(record) {
        ...
        if (!path.startsWith(managedRoot))
            throw new Error('document index contains an unsafe managed path');
        return path;
    }
    readBody(record) {
        ...
        while (descriptor === undefined) {
            try {
                ...
                if (Date.now() >= deadline)
                    throw new Error(`timed out waiting for document lock: ${this.lockPath}`);
...
export class DocumentManager {
    ...
    now;
    controllers = new Map();
    constructor(limitBytes = DOCUMENTS_ACTIVE_LIMIT_BYTES, now = () => new Date()) {
        this.limitBytes = limitBytes;
        this.now = now;
    }
    ...
    forAgent(agent) {
        ...
        if (cwd === undefined || cwd.trim() === '')
            throw new Error('the current DSH session has no workspace for Mnemon Documents');
