...
function turnMemoryActivity(events, turn) {
    ...
    for (const event of events) {
        if (event.type !== 'tool/call' || eventTurn(event) !== turn || typeof event.data.name !== 'string')
            continue;
        const name = event.data.name;
        if (!name.startsWith('mnemon_'))
            continue;
        names.push(name);
        if (MNEMON_READ_TOOLS.has(name))
            recalls += 1;
        else if (MNEMON_WRITE_TOOLS.has(name))
            writes += 1;
        if (name === 'mnemon_document_search')
            documentSearches += 1;
    }
    if (names.length === 0)
        return null;
