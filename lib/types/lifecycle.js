...
import { TurnActivityProjection } from "./activity.js";
import { registerAgentRuntimeMemoryContext } from "./guidance.js";
...
function assistantMessageText(events, messageId) {
    for (const event of events) {
        ...
        if (message === undefined || typeof message !== 'object' || message.id !== messageId)
            continue;
...
export class MnemonLifecycle {
    ...
    async supervise(sessionId, content, idempotencyKey, signal = new AbortController().signal) {
        ...
        if (normalizedKey !== undefined && normalizedKey.length > 200)
            throw new Error('idempotency key is too long (max 200 characters)');
        const execute = async () => {
        ...
        if (this.supervisedWritebacks.size >= 256) {
            ...
            if (oldest !== undefined)
                this.supervisedWritebacks.delete(oldest);
        }
        const result = execute();
        this.supervisedWritebacks.set(replayKey, { content: normalizedContent, result });
        void result.catch(() => {
            if (this.supervisedWritebacks.get(replayKey)?.result === result)
