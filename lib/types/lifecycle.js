...
function guidedReminder(config) {
    if (config.recallMode === 'guided' && config.writebackMode === 'guided')
        return '[MNEMON] Call mnemon_recall only when prior durable context matters; call mnemon_remember only for new, explicit, reusable user information. Otherwise call neither.';
    if (config.recallMode === 'guided')
        return '[MNEMON] Call mnemon_recall only when prior durable context matters; otherwise continue without recalling.';
    if (config.writebackMode === 'guided')
        return '[MNEMON] Call mnemon_remember only for new, explicit, reusable user information; otherwise continue without writing memory.';
    return undefined;
}
