...
//#endregion
//#region lib/types/live-runtime.js
/**
...
function createReadHandler(input, lifecycle, runtimeMemory, storage) {
...
function createWriteHandler(input, lifecycle, runtimeMemory) {
	return async (endpoint, rawPayload) => {
		try {
			const payload = object$1(rawPayload);
...
function registerRpc(connection, input, lifecycle, runtimeMemory, storage, packs) {
	...
	if (packManager !== void 0) connection.rpc.handle(MNEMON_PACK_CHANNEL, createPackHandler(packManager, () => config.writeEnabled), { authority: "loopback" });
}
//#endregion
...
function registerTools(ctx, serviceOrSource, coordinator, runtimeMemory, documents) {
	...
	ctx.tools.register(definition({
		...
		execute: (args, exec) => isSubagent(exec.agent) ? runtimeFor(exec).service.createBody(args, exec.signal) : coordinator.write(requireAgent(exec), "create-memory-body", args, exec.signal),
		presentCall: () => ({
			card: "generic",
...
//#endregion
export { Config, DocumentManager, InteractionConfig, LiveMnemonRuntime, MnemonLifecycle, MnemonPackManager, MnemonService, MnemonSubagentCoordinator, RuntimeMemoryController, StorageScopeInspector, apply, createRunner, createRuntimeGraph, inject, name, resolveConfig, resolveInteractionConfig };
