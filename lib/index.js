...
const ROUTING_GUIDANCE = "Use Mnemon only by need: call mnemon_recall when prior durable context may matter, and call mnemon_remember only for new, explicit, reusable user information. Otherwise call neither. Treat recall as fallible and writes as complete only after a tool receipt.";
...
catalog_json: ${JSON.stringify(catalog.items)}`;
	...
	async delegate(parent, operation, label, prompt, tools, outputSchema, signal, preferredProvider = "spawn") {
		...
		try {
			run = await this.subagents.start(provider, {
				...
				persona: operation === "review" ? "You are Mnemon's conservative idle checkpoint reviewer. Inspect the inherited completed conversation, use only the supplied Mnemon tools, default to no mutation, and call the structured output tool exactly once. Never delegate again." : "You are Mnemon's bounded memory worker. Use only the supplied Mnemon tools and evidence. Perform the requested retrieval or mutation, keep raw memory out of unrelated context, then call the structured output tool exactly once. Never delegate again."
			});
...
function guidedReminder(config) {
	if (config.recallMode === "guided" && config.writebackMode === "guided") return "[MNEMON] Call mnemon_recall only when prior durable context matters; call mnemon_remember only for new, explicit, reusable user information. Otherwise call neither.";
	if (config.recallMode === "guided") return "[MNEMON] Call mnemon_recall only when prior durable context matters; otherwise continue without recalling.";
	if (config.writebackMode === "guided") return "[MNEMON] Call mnemon_remember only for new, explicit, reusable user information; otherwise continue without writing memory.";
}
...
var MnemonAgentLifecycle = class {
	...
	scheduleIdleReview(turn) {
		...
		this.idleReviewTimer = setTimeout(() => {
			...
			if (!this.agent.session.events.some((event) => event.type === "turn/end" && eventTurn(event) === turn)) return;
			this.runIdleReview();
