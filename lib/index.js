...
const ROUTING_GUIDANCE = `## Mnemon memory bodies
...
var MnemonSubagentCoordinator = class {
	...
	recallResult(query, mode, provider, runId, result) {
		...
		const results = Array.isArray(value.results) ? value.results.map(insight).filter((entry) => entry !== void 0) : [];
...
function recallText(result) {
	if (result.results.length === 0) return void 0;
...
function incomingText(messages) {
	...
	return value.length <= 8e3 ? value : `${value.slice(0, 7999)}…`;
}
...
var MnemonAgentLifecycle = class {
	...
	async preStep(payload, next) {
		...
		if (this.config.recallMode === "guided") try {
			...
			if (query !== "") {
				...
				if (recalled !== void 0) sections.push(recalled);
		...
		try {
			...
			switch (endpoint) {
				...
				case "entities": {
					...
					if (entity === "" || lifecycle === void 0) return success$1(await service.entities(entity || void 0, limit));
					const base = await service.entities();
					const recalled = await lifecycle.recall(String(payload.sessionId ?? ""), {
						query: entity,
						intent: "ENTITY",
						...limit === void 0 ? {} : { limit }
					});
				...
				case "search": {
					...
					return success$1(lifecycle === void 0 ? await service.search(request) : await lifecycle.recall(String(payload.sessionId ?? ""), request));
				}
				case "related": return success$1(lifecycle === void 0 ? await service.related(String(payload.id ?? ""), payload.depth === void 0 ? 2 : Number(payload.depth), payload.edge, void 0, payload.memoryBodyId === void 0 ? void 0 : String(payload.memoryBodyId)) : (await lifecycle.related(String(payload.sessionId ?? ""), String(payload.id ?? ""), payload.memoryBodyId === void 0 ? void 0 : String(payload.memoryBodyId))).results);
				default: return {
					ok: false,
					error: {
						code: "not-found",
...
function registerTools(ctx, service, coordinator) {
	ctx.tools.register(definition({
		...
		presentCall: () => ({
			card: "generic",
			title: "Inspect Mnemon memory bodies",
			kind: "search"
		}),
		presentResult: () => ({
			card: "generic",
			title: "Mnemon memory bodies ready"
		})
	}));
	ctx.tools.register(definition({
		name: "mnemon_recall",
		...
		parameters: {
			...
			properties: {
				...
				memoryBodyIds: {
					type: "array",
					items: { type: "string" },
					description: "One or more active memory-body ids. Omit to search every active body.",
					maxItems: 20
				}
			},
			required: ["query"]
	...
	ctx.tools.register(definition({
		...
		parameters: {
			...
			properties: {
				...
				memoryBodyId: {
					type: "string",
					description: "Active memory body that returned this insight id."
				}
			},
			required: ["id"]
		},
	...
	ctx.tools.register(definition({
		...
		parameters: {
			...
			properties: {
				...
				memoryBodyId: {
					type: "string",
					description: "Target memory-body id. Required unless exactly one body is active."
				}
			},
			required: ["content"]
		},
	...
	ctx.tools.register(definition({
		...
		presentCall: () => ({
			card: "generic",
			title: "Create Mnemon memory body",
			kind: "edit"
		}),
		presentResult: () => ({
			card: "generic",
			title: "Mnemon memory body created"
		})
	}));
	ctx.tools.register(definition({
		...
		presentCall: () => ({
			card: "generic",
			title: "Update Mnemon memory body",
			kind: "edit"
		}),
		presentResult: () => ({
			card: "generic",
			title: "Mnemon memory body updated"
		})
	}));
	ctx.tools.register(definition({
		...
		presentCall: () => ({
			card: "generic",
			title: "Merge Mnemon memory bodies",
			kind: "edit"
		}),
		presentResult: () => ({
			card: "generic",
			title: "Mnemon memory bodies merged"
		})
	}));
}
//#endregion
