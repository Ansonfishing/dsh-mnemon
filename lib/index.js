...
import { accessSync, closeSync, constants, existsSync, fstatSync, lstatSync, mkdirSync, mkdtempSync, openSync, readFileSync, readdirSync, renameSync, rmSync, statSync, writeFileSync } from "node:fs";
import { homedir, tmpdir } from "node:os";
import { spawn } from "node:child_process";
import { strToU8, unzipSync, zipSync } from "fflate";
//#region lib/types/config-values.js
...
function resolveConfig(config = {}) {
	...
	const storageScope = config.storageScope ?? (legacyDataDir === void 0 && customPacks.length === 0 ? "global" : "custom");
	const selectedPack = requestedPackId === void 0 ? customPacks.find((pack) => pack.dataDir === legacyDataDir) ?? (customPacks.length === 1 ? customPacks[0] : void 0) : customPacks.find((pack) => pack.id === requestedPackId);
	if (requestedPackId !== void 0 && selectedPack === void 0) throw new Error(`dsh-mnemon: unknown custom Pack: ${requestedPackId}`);
	if (storageScope === "custom" && selectedPack === void 0) throw new Error("dsh-mnemon: a custom Pack is required when storageScope is custom");
	...
	return {
		...
		...selectedPack === void 0 ? {} : { customPackId: selectedPack.id },
		customPacks,
		...store === void 0 ? {} : { store },
...
function assistantMessageText(events, messageId) {
	for (const event of events) {
		...
		const text = (Array.isArray(message.blocks) ? message.blocks : []).filter((block) => block.type === "text" && typeof block.text === "string").map((block) => String(block.text)).join("\n\n").trim();
		return text === "" ? null : {
			messageId,
			text
		};
		...
		try {
			...
			switch (endpoint) {
				...
				case "supervise":
					...
					return success$1(await lifecycle.supervise(String(payload.sessionId ?? ""), String(payload.content ?? ""), payload.idempotencyKey === void 0 ? void 0 : String(payload.idempotencyKey)));
				case "document":
					if (lifecycle === void 0) throw new Error("Mnemon Documents require lifecycle integration");
					{
...
function createPackHandler(manager, writeEnabled = true) {
	return async (endpoint, rawPayload) => {
		try {
			const payload = object$1(rawPayload);
			if (endpoint === "target") return success$1(manager.target());
			if (endpoint === "export") return success$1(await manager.exportPack(String(payload.scope ?? "")));
			...
			if (endpoint === "import") {
				...
				return success$1(await manager.importPack(String(payload.base64 ?? ""), {
					mode: String(payload.mode ?? ""),
					...Array.isArray(payload.components) ? { components: payload.components.map(String) } : {}
				}));
			}
...
function registerRpc(connection, service, lifecycle, runtimeMemory, storage, packs) {
	...
	if (packs !== void 0) connection.rpc.handle(MNEMON_PACK_CHANNEL, createPackHandler(packs, service.config.writeEnabled), { authority: "loopback" });
}
//#endregion
...
function normalizeInsight(value) {
	...
	const core = record$2(item.insight) ?? item;
...
var MnemonService = class {
	...
	async search(request, signal) {
		...
		for (const body of bodies) {
			...
			const wrapper = record$2(payload);
	...
	async bodyStatus(body, signal) {
		try {
			const status = record$2(await this.runner.runJson(["status"], {
				...signal === void 0 ? {} : { signal },
				store: body.id
			}));
	...
	parseStats(status) {
		const byCategoryRecord = record$2(status.by_category) ?? {};
		const byCategory = {};
	...
	async allInsights(body, signal, readonly = false) {
		...
		return (Array.isArray(payload) ? payload : Array.isArray(record$2(payload)?.results) ? record$2(payload).results : []).map(normalizeInsight).filter((entry) => entry !== void 0);
	}
	readBodies(ids) {
...
const MNEMON_PACK_MIME = "application/vnd.mnemon.pack+zip";
...
var MnemonPackManager = class {
	...
	target() {
		return {
			...
			...this.config.customPackId === void 0 ? {} : { customPackId: this.config.customPackId }
		};
	}
	async exportPack(scope) {
		...
		return this.runner.withExclusive(async () => {
			...
			return withLocks(this.root, components, () => {
				...
				return {
					fileName: `mnemon-${scope}-${exportedAt.replace(/[:.]/gu, "-").replace("T", "_").replace("Z", "")}.mnemonpack`,
...
//#endregion
export { Config, DocumentManager, InteractionConfig, MnemonLifecycle, MnemonPackManager, MnemonService, MnemonSubagentCoordinator, RuntimeMemoryController, StorageScopeInspector, apply, createRunner, inject, name, resolveConfig, resolveInteractionConfig };
