...
function createReadHandler(service) {
	return async (endpoint, rawPayload) => {
		try {
			...
			switch (endpoint) {
				case "status": return success(await service.status());
				case "search": return success(await service.search({
				...
				case "related": return success(await service.related(String(payload.id ?? ""), payload.depth === void 0 ? 2 : Number(payload.depth), payload.edge));
				default: return {
					ok: false,
					error: {
						code: "not-found",
...
function createWriteHandler(service) {
	return async (endpoint, rawPayload) => {
		try {
			const payload = object(rawPayload);
			switch (endpoint) {
				case "remember": return success(await service.remember({
				...
				case "link": return success(await service.link(String(payload.sourceId ?? ""), String(payload.targetId ?? ""), payload.type, payload.weight === void 0 ? .5 : Number(payload.weight), payload.reason === void 0 ? void 0 : String(payload.reason)));
				case "forget": return success(await service.forget(String(payload.id ?? "")));
				default: return {
					ok: false,
					error: {
						code: "not-found",
...
function apply(rawContext, config = {}) {
	...
	if (resolved.tabEnabled) ctx.inject(["connection"], (webContext) => {
