...
function findMnemonCommand(config) {
	...
	if (envPath !== void 0 && envPath !== "") {
		...
		if (executable$1(path)) return path;
	}
...
//#endregion
//#region lib/types/version-updates.js
...
function profileFromAncestor(packageManifestPath) {
	...
	for (let depth = 0; depth < 12; depth++) {
		...
		if (profile?.name?.startsWith("dsh-profile-") === true) return {
			mode: isLinkSpec(dependencySpec(profile)) ? "link" : "npm",
...
function registerRpc(connection, input, lifecycle, runtimeMemory, storage, packs, versions) {
