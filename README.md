...
打开 DSH 的“设置 -> 插件 -> Mnemon”独立配置页选择存储范围，再进入会话的“记忆系统”Tab 创建或激活记忆体。存储配置重启后生效，对话呈现设置保存后实时生效；切换范围不会自动迁移、合并或删除旧数据。
...
记忆也会在对话流中直接体现（对话内交互，**默认关闭**，可在「设置 → 插件 → Mnemon → 对话内交互」中逐项开启，保存即实时生效）：
