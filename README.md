...
主 Agent 不需要接收完整目录、原始检索过程或写回推理。主模型按需调用的召回与写操作使用有界的 `spawn` 子 Agent；当 turn 持续空闲达到阈值时，插件才使用 `fork` 子 Agent 继承主 Agent 的完整已完成 checkpoint，进行一次保守的 remember / forget 复核。只有精简证据或结构化回执会回到主上下文；存储、四图索引、衰减和去重仍由本地 Mnemon CLI 确定性执行。
...
|---|---|---|---|
| 每轮开始 | `agent/pre-step` | 只注入一句按需 recall / remember 提醒，不读取目录、不执行记忆工具 | 主模型自行判断是否发起召回或沉淀 |
| 显式工具/命令 | `ctx.subagents.start('spawn', …)` | 在受限 Mnemon 工具集合内完成一次语义操作 | 结构化召回结果或写入回执 |
| turn 结束并持续空闲 | `agent/turn-stopping` → debounce → `fork` | 继承截至最新 `turn/end` 的完整主 Agent checkpoint，保守判断是否需要一次 remember 或 forget | 不注入复核推理，不唤醒主 Agent；新 turn 会取消复核 |
...
4. 形成稳定决策、偏好、流程或难得经验后，主 Agent 可主动调用 `mnemon_remember`；空闲审查只作为保守兜底。
...
测试覆盖记忆体目录迁移、独立 Store 路由、激活读边界、写后激活、非破坏性合并、子 Agent 工具隔离、pre-step 仅提醒、turn 空闲 debounce、完整 checkpoint `fork` 审查与新 turn 取消、WebUI 直连检索、证据限定 Agent 问答、RPC 权限、只读模式、多记忆体图谱和六区工作台。发布前还应使用隔离 `MNEMON_DATA_DIR` 和独立端口 DSH，通过真实 WebUI 对话验证普通任务不触发 recall、历史问题由模型自主召回、显式稳定信息由模型自主沉淀、空闲后恰好一次复核、跨记忆体读取、简单显式边、状态计数和最终 CLI recall。
