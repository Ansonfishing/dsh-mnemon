...
目录卡片先展示名称和路由说明，Provider 标记紧邻 ID 与健康状态，右上角仍是熟悉的读取激活开关。创建入口没有增加新的一级概念：仍然是“创建记忆体”，只在弹窗中选择 **Mnemon Native**（默认、官方原生）或 **OpenViking**（连接已有服务）。Native 卡片保留统计、编辑和删除；OpenViking 卡片展示远程 URI、异步提炼语义以及“断开”。断开只移除连接，不删除远程数据。
