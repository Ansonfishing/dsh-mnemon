...
目录卡片先展示名称和路由说明，Provider 标记紧邻 ID 与健康状态，右上角仍是熟悉的读取激活开关。创建入口没有增加新的一级概念：仍然是“创建记忆体”，并默认保持“手动指定”，在弹窗中选择 **Mnemon Native**（官方原生）或 **OpenViking**（连接已有服务）。当前对话可用时，也可以主动切换“智能选择”：用数据边界和必需能力设置硬规则，用本地/共享软偏好与策略 Prompt 引导隔离子 Agent。只有多个合格候选时才调用模型，凭据不会进入模型上下文。

Native 卡片保留统计、编辑和删除；OpenViking 卡片展示远程 URI、异步提炼语义以及“断开”。智能创建的卡片还会展示“规则自动确定”或“Agent 智能选择”、置信度和简短理由。断开只移除连接，不删除远程数据。

下方关系图聚合全部已激活记忆体。Mnemon Native 提供完整关系；OpenViking 当前以有界无边节点参与内容投影，不伪造关系。布局、拖拽和重置只影响当前浏览器展示。
...
- 只有 Provider 支持时才显示“查看关联”和“忘记”；OpenViking 当前不显示这两项。
...
- 删除 Mnemon Native 记忆体等物理危险操作需要独立确认；OpenViking 使用明确的“断开”确认且不触碰远程数据。“忘记”当前是 Mnemon Native 长期语义软删除。
