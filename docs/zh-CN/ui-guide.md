...
目录卡片先展示名称和路由说明，Provider 标记紧邻 ID 与健康状态，右上角仍是熟悉的读取激活开关。创建入口没有增加新的一级概念：仍然是“创建记忆体”，并默认保持“手动指定”，在弹窗中选择 **Mnemon Native**（官方原生、优先）或 8 种三方引擎之一。当前对话可用时，也可以主动切换“智能选择”：用数据边界和必需能力设置硬规则，用本地/共享软偏好与策略 Prompt 引导隔离子 Agent。只有多个合格候选时才调用模型，凭据不会进入模型上下文。
