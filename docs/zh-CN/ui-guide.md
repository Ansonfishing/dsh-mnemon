...
本指南按 v0.3.0 默认 `sidebar` 体验和真实用户路径编排。v0.2 的既有交互流程在本次升级中刻意保持不变；兼容截图来自 v0.2.0 的 1600×900 实机页面，v0.3 新增的 Layer 拓扑控制则在“设置”部分单独说明。名称、数量和内容会随本地数据变化。
...
- Layer 拓扑卡来自运行中的 Catalog；可以分别关闭 Runtime、Documents、Memory Spaces，或调整召回、写入、投影和维护的“关闭 / 仅手动 / 自动”参与模式；
...
关闭 Layer 不删除数据。扩展贡献的新 Layer 首次出现时默认关闭且仅手动；保存整份拓扑前，当前运行代继续服务，候选配置验证失败也不会产生半切换。
