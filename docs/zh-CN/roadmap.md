...
- [ ] **Cordis / DSH 能力契约测试**：围绕服务注入、热重载与 dispose、延迟注册工具建立宿主集成测试；特别验证 Mnemon 经过 schema 校验的一次性结果工具在子 Agent 中保持可达，同时 `toolFilter` 仍隐藏所有未列入白名单的能力，并以 [issue #14](https://github.com/Grivn/dsh-mnemon/issues/14) / [PR #17](https://github.com/Grivn/dsh-mnemon/pull/17) 作为回归场景。
