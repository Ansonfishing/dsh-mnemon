...
- DSH 0.1.1-rc.1 基线所需的 Node.js `^22.19.0 || >=24.0.0`；
...
本文流程以 dsh-mnemon v0.3.0、DSH 0.1.1-rc.1 和 Mnemon 0.2.3 为推荐基线；部分保持兼容的界面截图拍摄于 v0.2.0。DSH rc.1 使用 `Promise.withResolvers` 和 Node Zstd API，因此 Node 20 无法启动完整 profile。在本版本发布时，DSH 0.1.1-rc.1 位于 npm `next` 标签，`latest` 仍指向 0.1.0-rc.7。升级前先备份，并在隔离目录重复本页验证。
