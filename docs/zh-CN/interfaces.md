...
Sidebar 使用贴近 DSH 官方面板的极简皮肤：标题为“记忆系统”，不展示 Mnemon Logo、顶部统计或导航装饰，只保留紧凑的文字标签。Buildin 保持原有 Mnemon 品牌头部、统计和分组导航视觉不变。两种形态的导航都覆盖八个页面：「状态」；三层存储「运行时、记忆体、档案」；读写工具「沉淀、检索、实体、内容」。默认进入「状态」页。
