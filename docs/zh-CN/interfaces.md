...
主要 Web 界面跟随 DSH 全局语言和明暗主题。

| 页面 | 用途 | 调用边界 |
|---|---|---|
| 总览 | 记忆体目录、激活开关、多空间实时图谱、节点检查 | 确定性 RPC 读取；开关为写 RPC |
| 运行时 | 查看和维护 USER / MEMORY 热记忆及容量 | 普通 mutation 确定性；容量维护可启动 worker |
| 档案 | 搜索、阅读、创建、更新、冷归档 Documents | 搜索/编辑走控制层；冷归档启动 worker |
...
| 状态 | CLI、存储域、记忆体、Documents、生命周期和 worker 诊断 | 聚合读取 |
