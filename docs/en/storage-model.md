...
provider      mnemon-native or openviking
location      local mnemon.db, or remote endpoint + target URI
```
...
- Writes may target any registered space with `remember`; OpenViking waits for asynchronous extraction to finish before returning a settled receipt.
...
OpenViking visibility is determined by the service, target URI, and identity headers. Neither sharing mode extends to `runtime/` or `documents/`; “shared third-tier memory” must not be presented as automatic sharing of the complete DSH context.

## Four Relationship Types

Mnemon Native preserves `temporal`, `semantic`, `causal`, and `entity` relationships. OpenViking is currently projected into the graph as bounded disconnected nodes; the adapter never invents edges. The UI hides Related and Forget where the provider does not support them.
