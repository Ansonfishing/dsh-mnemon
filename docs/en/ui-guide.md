...
On DSH 0.1.1-rc.1, model capabilities flow into this picker. Image-capable routes carry an **Image input** label, including the first-party `deepseek-official/deepseek-v4-flash-vision-exp` model. The label describes model capability; current Mnemon background jobs still send text-only prompts.
