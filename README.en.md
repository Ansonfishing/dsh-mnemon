...
- **Save to memory**: every finalized assistant reply carries a memory icon aligned with the native action strip; clicking first opens an editable confirmation, and only confirmation routes the candidate through the isolated memory subagent (judgment, dedupe, distillation) without filling the main conversation context.
