...
async function execute(service: MnemonService, invocation: CommandInvocation): Promise<CommandResult> {
...
export function createMnemonCommand(service: MnemonService): CommandDefinition {
  return {
    name: 'mnemon',
    description: '查看、召回或管理 Mnemon 外置记忆',
    input: { hint: '[status|recall <查询>|related <ID>|remember <内容>|forget <ID>]' },
    handler: invocation => execute(service, invocation).catch((reason: unknown) => ({
      kind: 'error',
...
export function registerCommands(commands: CommandService, service: MnemonService): void {
