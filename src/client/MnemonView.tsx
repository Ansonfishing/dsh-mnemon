...
function ReadSourcePanel(props: {
  ...
  const content = (source: MemoryReadSource) => <>
    ...
    <span className={css.readSourceIdentity}><strong>{source.memoryBodyName}</strong><small>{source.providerLabel} · {t(`readSources.model.${source.providerId}` as MnemonKey)}</small></span>
  ...
  return <section className={css.readSources} aria-label={props.title}>
    ...
    <div>{props.sources.map(source => props.onSelect === undefined
      ? <article key={source.memoryBodyId} className={css.readSourceCard} data-mode={source.mode} data-status={source.status} title={source.hint}>{content(source)}</article>
      : <button key={source.memoryBodyId} type="button" className={css.readSourceCard} data-mode={source.mode} data-status={source.status} data-selected={props.selectedBodyId === source.memoryBodyId || undefined} title={source.hint} onClick={() => props.onSelect?.(source.memoryBodyId)}>{content(source)}</button>,
    )}</div>
  </section>
}
...
function OverviewPage(props: { client: MnemonClient; revision: number; writeEnabled: boolean; agentAvailable: boolean; fallbackBodies: MemoryBodyView[]; fallbackDirectory: string | undefined; catalogKnown: boolean; onMutate: () => void; onExplore: (query: string) => void }): JSX.Element {
  ...
  const bodyCreateForm = <form className={css.bodyEdit} onSubmit={event => void create(event)}>
    ...
    <div className={css.bodyEditActions}><button type="button" className={css.ghostButton} disabled={creating} onClick={() => setCreatingBodyOpen(false)}>{t('common.cancel')}</button><button type="submit" className={css.primaryButton} disabled={creating || bodyName.trim() === '' || bodyDescription.trim() === '' || (placementMode === 'automatic' && !props.agentAvailable) || (placementMode === 'manual' ? !providerDraftComplete(selectedProvider, providerDrafts[bodyProviderId]) : automaticProviderIds.some(id => !providerDraftComplete(providers.find(provider => provider.id === id), providerDrafts[id])))}>{creating ? t('overview.creating') : t('overview.createAction')}</button></div>
  </form>
