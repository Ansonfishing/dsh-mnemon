...
export const zh = {
  ...
  'overview.metadataDescription': '选择一个或多个已激活记忆体。子 Agent 将分别使用其 Provider 原生检索读取内容，并生成更清晰的标题与说明。',
  'overview.metadataUnavailable': '需要当前会话中的可用 Agent 才能生成元信息',
...
export const en = {
  ...
  'overview.metadataDescription': 'Select one or more active Memory Spaces. A subagent will query each Provider through its native retrieval path and generate a clearer title and description.',
  'overview.metadataUnavailable': 'A live Agent in the current session is required to generate metadata',
