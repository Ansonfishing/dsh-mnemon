...
export const zh = {
  ...
  'config.embeddingDescription': '让 DSH Host 为每次 Mnemon CLI 调用注入 Ollama endpoint 与模型；Finder 或 Dock 启动的 macOS 桌面端无需依赖 shell 环境。',
  'config.embeddingManaged': '由 DSH 管理嵌入配置',
  'config.embeddingManagedHint': '开启后以这里保存的值覆盖 Mnemon 子进程环境；关闭后继续沿用 Host 环境与 Mnemon 默认值。',
  'config.embeddingEndpoint': 'Ollama Endpoint',
  ...
  'config.embeddingSecurity': 'Mnemon 会把记忆与查询正文发送到该 Endpoint。远程 HTTP 会明文传输，请仅使用受信任的本地地址，或为远程服务配置 HTTPS。',
  'config.embeddingTest': '测试状态',
  'config.embeddingNotTested': '尚未检查当前 Mnemon Store 的连接与嵌入覆盖率。',
  'config.embeddingTesting': '正在通过 Mnemon 检查 Ollama 与嵌入覆盖率…',
  'config.embeddingSaveBeforeTest': '请先保存嵌入配置，再测试实际运行值。',
  'config.embeddingTestUnavailable': '当前 DSH Host 不提供嵌入状态检查。',
  'config.embeddingStatusAvailable': 'Ollama 可用 · {model} · 已嵌入 {embedded}/{total}（{coverage}）',
  'config.embeddingStatusUnavailable': 'Ollama 不可达 · {model} · 已嵌入 {embedded}/{total}（{coverage}）',
...
export const en = {
  ...
  'config.embeddingDescription': 'Have the DSH Host inject the Ollama endpoint and model into every Mnemon CLI call, including macOS Desktop launches from Finder or the Dock.',
  ...
  'config.embeddingSecurity': 'Mnemon sends memory and query text to this endpoint. Remote HTTP is unencrypted; use a trusted local address or HTTPS for a remote service.',
  ...
  'config.embeddingTesting': 'Checking Ollama and embedding coverage through Mnemon…',
  ...
  'config.embeddingStatusAvailable': 'Ollama available · {model} · {embedded}/{total} embedded ({coverage})',
  'config.embeddingStatusUnavailable': 'Ollama unreachable · {model} · {embedded}/{total} embedded ({coverage})',
