...
export const zh = {
  ...
  'overview.title': '记忆体',
  'overview.description': '创建并管理具体记忆体；每个记忆体使用一个已启用 Provider，并统一参与读取、路由与实时快照。',
  ...
  'overview.directory.description': '每张卡片是一份可独立启停的记忆体配置。“激活”决定它是否参与 DSH 读取与路由；服务、凭据和全局数据位置在“设置 → 记忆系统”管理。',
  ...
  'overview.createPlacementHint': '手动固定一个已启用 Provider，或让 Agent 在符合规则的候选中选择。',
...
export const en = {
  ...
  'overview.description': 'Create and manage concrete Memory Spaces. Each one uses an enabled provider and participates in unified reads, routing, and live snapshots.',
  ...
  'overview.directory.description': 'Each card is an independently switchable Memory Space configuration. Active spaces participate in DSH reads and routing; manage services, credentials, and global data locations in Settings → Memory System.',
  ...
  'overview.createPlacementHint': 'Pin one enabled provider, or let the agent choose among candidates that satisfy your rules.',
