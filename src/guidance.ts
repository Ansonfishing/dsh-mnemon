...
export const ROUTING_GUIDANCE = 'Use memory only when needed. Search active Mnemon Documents for substantial project records. Call mnemon_recall only for missing durable history or exact prior details; never infer a missing historical rule. Put only new user-supplied reusable facts or explicit save/correction requests in mnemon_runtime_memory; never cache retrieved evidence. A write exists only after its receipt.'
