...
export class MnemonService {
  ...
  private async graphForBody(body: MemoryBody, signal?: AbortSignal): Promise<MemoryGraphSnapshot> {
    const html = await this.runner.runText(['viz', '--format', 'html', '--output', '-'], { ...(signal === undefined ? {} : { signal }), store: body.id })
    return parseMemoryGraph(html)
  }

  private async allInsights(body: MemoryBody, signal?: AbortSignal): Promise<Insight[]> {
    const payload = await this.runner.runJson(['recall', '', '--basic', '--limit', '100000'], { ...(signal === undefined ? {} : { signal }), store: body.id })
